...
static int commit_cas(
    ...
    for (size_t i = 0; i < conns.size(); ++i) {
        ...
        wr.sg_list    = &sge;
        wr.num_sge    = 1;
        wr.wr.rdma.remote_addr  = conns[i].addr + (slot * 8);
...
static bool learn_majority(
    ...
    // Wait for ALL replicas (not just quorum)
    int reads_done = 0;
    ibv_wc wc{};
...
static void synra_reset(
    LocalState* state, const uint32_t client_id,
    const std::vector<RemoteNode>& conns, ibv_cq* cq, const ibv_mr* mr
) {
    uint64_t current_idx = discover_frontier(state, 0, conns, cq, mr);
    ...
    *write_val = static_cast<uint64_t>(client_id);
    ...
    for (size_t i = 0; i < conns.size(); ++i) {
        ...
        wr.sg_list    = &sge;
        wr.num_sge    = 1;
        wr.wr.rdma.remote_addr = conns[i].addr + (next_slot * 8);
        wr.wr.rdma.rkey        = conns[i].rkey;
    ...
    advance_frontier(state, next_slot, conns, mr);
}
...
uint64_t TasStrategy::acquire(Client& client, const int op_id, uint32_t /*lock_id*/) {
    ...
    while (true) {
        const uint64_t max_val = discover_frontier(state, op_id, conns, cq, mr);

        if (max_val % 2 != 0) continue;
        const uint64_t next_slot = max_val + 1;

        if (commit_cas(state, op_id, next_slot, client.id(), conns, cq, mr) >= static_cast<int>(QUORUM)) {
            advance_frontier(state, next_slot, conns, mr);
            return next_slot;
        }

        if (learn_majority(state, op_id, next_slot, client.id(), conns, cq, mr)) {
            advance_frontier(state, next_slot, conns, mr);
            return next_slot;
        }
    }
