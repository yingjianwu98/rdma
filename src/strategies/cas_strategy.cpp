...
static constexpr uint64_t TAG_REPLICATE = 0x3;
    ...
    while (true) {
        ...
        if (result == expected) {
            ...
            for (size_t i = 0; i < conns.size(); ++i) {
                ...
                rep_wr.wr_id = make_wr_id(op_id, TAG_REPLICATE, i);
            ...
            while (acks < static_cast<int>(QUORUM)) {
                int pulled = ibv_poll_cq(cq, 16, wcs);
                for (int j = 0; j < pulled; ++j) {
                    if (wcs[j].status != IBV_WC_SUCCESS) continue;
                    if (wr_tag(wcs[j].wr_id) == TAG_REPLICATE &&
                        wr_seq(wcs[j].wr_id) == static_cast<uint32_t>(op_id)) {
                        acks++;
                    }
                }
            }
