...
void Server::start(uint16_t port) {
    std::cout << "[Server " << node_id_ << "] Starting initialization...\n" << std::flush;
    ...
    if (rdma_create_id(ec_, &listener_, nullptr, RDMA_PS_TCP))
        throw std::runtime_error("rdma_create_id failed");
    std::cout << "[Server " << node_id_ << "] CM ID created\n" << std::flush;
    ...
    std::cout << "[Server " << node_id_ << "] Lock headers initialized\n" << std::flush;

    std::cout << "[Server " << node_id_ << "] Getting cluster config...\n" << std::flush;
    const size_t num_nodes = CLUSTER_NODES.size();
    const uint32_t num_clients = expected_clients();
    std::cout << "[Server " << node_id_ << "] Cluster config: " << num_nodes << " nodes, " << num_clients << " clients\n" << std::flush;
    ...
    // ── Phase 2: Accept remaining connections (blocking) ──
    ...
    while (higher_connected < expect_higher || clients_connected < num_clients) {
        // Block waiting for incoming connections
        rdma_cm_event* event = nullptr;
        ...
        // Process using the same helper logic (manually inline for blocking case)
        rdma_cm_id* new_id = event->id;
        auto* incoming = static_cast<const ConnPrivateData*>(
            event->param.conn.private_data);
