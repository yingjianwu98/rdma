...
void MuFollower::connect_to_leader(const std::string& leader_ip, uint16_t port) {
    ...
    log_pool_ = allocate_rdma_buffer();
    ...
    log_mr_ = ibv_reg_mr(pd_, log_pool_, FINAL_POOL_SIZE,
    ...
    priv.addr    = reinterpret_cast<uintptr_t>(log_pool_);
    priv.rkey    = log_mr_->rkey;
...
void MuFollower::run() {
    ...
    auto* log = static_cast<char*>(log_pool_);
    ...
    while (true) {
        ...
        if (n > 0 && wc.status == IBV_WC_SUCCESS) {
            uint32_t received = be32toh(wc.imm_data);
