...
void post_advance_acquire(Client& client, TasOpCtx& op, const RegisteredTasBuffers& buffers) {
    const auto& conns = client.connections();
    auto* mr = client.mr();
...
void post_advance_release(Client& client, TasOpCtx& op, const RegisteredTasBuffers& buffers) {
    const auto& conns = client.connections();
    auto* mr = client.mr();
    ...
    while (completed < NUM_OPS_PER_CLIENT) {
        ...
        for (int i = 0; i < polled; ++i) {
            ...
            if (phase == TasPhase::advance_acquire) {
                ...
                if (op.advance_acquire_pending == 0) {
                    post_advance_release(client, op, buffers);
                }
                continue;
            }
            ...
            if (phase == TasPhase::commit) {
                if (op.commit_wins >= SUPER_QUORUM) {
                    ...
                    post_advance_acquire(client, op, buffers);
                } else {
