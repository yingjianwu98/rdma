...
void Client::connect_peers(uint16_t peer_port) {
    ...
    for (uint32_t target = 0; target < id_; ++target) {
        // all clients are on the same machine — connect to localhost
        ...
        for (int attempt = 0; attempt < 60; ++attempt) {
            try {
                ...
                ev = wait_for_event(ec_, RDMA_CM_EVENT_ESTABLISHED, "PEER_ESTABLISHED");
