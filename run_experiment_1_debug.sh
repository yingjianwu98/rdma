...
# Run just Experiment 1 with debug output
...
NUM_OPS=1000
echo "========================================="
echo "Running Experiment 1: $NUM_OPS ops"
echo "========================================="

# Update NUM_OPS in common.h
sed -i '' "s/constexpr size_t NUM_OPS = [0-9]*;.*/constexpr size_t NUM_OPS = $NUM_OPS;  \/\/ Experiment 1/" include/rdma/common.h

# Commit and push
git add -f include/rdma/common.h
git commit -m "Experiment 1: $NUM_OPS ops"
git push
...
ssh stevie98@apt121.apt.emulab.net "cd /local/rdma/build && sudo IS_CLIENT=1 MACHINE_ID=0 timeout 300 ./rdma 2>&1" | tee "/tmp/${TIMESTAMP}_${NUM_OPS}_debug.txt"

echo ""
echo "========================================="
echo "Results saved to: /tmp/${TIMESTAMP}_${NUM_OPS}_debug.txt"
echo "========================================="
